 ...
 * 作用： 统筹管理类型处理器的。（类型处理器是真正用于进行java类型与数据库类型映射的工具）
 * <p>
 * 1、类型处理器注册表的内部定义了集合来进行类型处理器的存取，同时定义了存取方法
 *
...
public final class TypeHandlerRegistry {
     ...
     * (1)枚举Map集合
     * (2)以JdbcType枚举类中枚举值为键创建的一种集合，这种集合先天存在键（枚举值），
     * 它是以数据库类型为键来保存类型处理器，
     * 亦即将类型处理器注册到对应的数据库类型上。
     ...
     * (1)嵌套Map集合。
     * (2)内层集合是以数据库类型为键保存处理器，
     * 外层集合为以Java类型来保存对应的数据库类型及其处理器，
     * 这个集合将三者联系起来，是真正进行三者对应关系匹配的集合。
     *
     * KEY1：JDBC Type
     * KEY2：Java Type
    ...
    /**
     * 所有 TypeHandler 的“集合”
     * 该集合中保存着所有的类型处理器，是以类型处理器的类类型为键值保存的，它可以统筹所有的类型处理器（带有统计的效果）
    ...
    //构造函数里注册系统内置的类型处理器
    // 对内就是该方法(三种重载的register方法)被类内部调用进行注册。
    // 对外就是该方法(register方法)被类外部的其他类进行调用而进行注册。
    // 对外情况就是XMLConfigBuilder类，它在构建Configuration对象时就会调用对外的注册方法，来将用户自定义的类型处理器注册到注册器中。
    ...
    //入口1：使用枚举集合（用于将类型处理器注册到对应的数据库类型。）
    ...
    public <T> void register(TypeHandler<T> typeHandler) {
        ...
        // 其次，若没有@MappedTypes注解修饰而且当 typeHandler 为 TypeReference 子类时，进行注册
    ...
    // 入口2：对应第二种嵌套集合（其中内层集合的键为null）
    // 该入口方法用于将类型处理器注册到对应的Java类型。
    // 这也是 对外入口2：指定Java类型与类型处理器的情况
    ...
    // 也是对外入口3：指定JavaType、JdbcType、TypeHandler三者的情况
